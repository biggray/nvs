...
function doCommand(args) {
	...
	switch (args[0]) {
        ...
        case 'proxy':
            if (help) return help('proxy');
			if (args[1] === '-d' || args[1] === 'rm' || args[1] === 'remove') {
                return require('./settings').removeProxy();
			} else if (args[1] === 'add' || args[1] === 'set') {
                return require('./settings').setProxy(args[1]);
            } else if (args[1] === 'get' || args[1] === 'show') {
                return require('./settings').getProxy();
			} else if (args[1]) {
				return require('./settings').setProxy(args[1]);
			} else {
				return require('./settings').getProxy();
